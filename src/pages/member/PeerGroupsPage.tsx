...
interface Membership {
  id: string; group_id: string; user_id: string; joined_at: string;
}
...
export default function PeerGroupsPage() {
  ...
  const { data: groups = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('peer_groups').select('*').order('name');
  ...
  const { data: myMemberships = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!user?.id) return [];
      const { data, error } = await supabase.from('peer_group_members').select('*').eq('user_id', user.id);
      if (error) throw error;
  ...
  // For joined groups, fetch peer members
  ...
  const { data: peerMembers = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from('peer_group_members').select('*').in('group_id', joinedGroupIds);
      if (error) throw error;
      return (data || []) as unknown as Membership[];
    },
  ...
  const { data: peerProfiles = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from('profiles').select('id, full_name').in('id', peerUserIds);
      if (error) throw error;
  ...
  const joinMutation = useMutation({
    mutationFn: async (groupId: string) => {
      const { error } = await supabase.from('peer_group_members').insert({ group_id: groupId, user_id: user!.id });
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-group-memberships'] });
      queryClient.invalidateQueries({ queryKey: ['peer-group-peers'] });
  ...
  const leaveMutation = useMutation({
    mutationFn: async (groupId: string) => {
      const { error } = await supabase.from('peer_group_members').delete().eq('group_id', groupId).eq('user_id', user!.id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-group-memberships'] });
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Peer Groups</h1>
          ...
          {groups.map((group, i) => {
            ...
            return (
              <motion.div key={group.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <Card className={`border-border/50 transition-all ${joined ? 'border-primary/30 shadow-sm' : ''}`}>
                            ...
                            <div key={peer.id} className="flex items-center gap-1.5">
                              <Avatar className="h-6 w-6">
                                <AvatarFallback className="text-[9px] bg-accent text-accent-foreground">
                                  {(profileMap.get(peer.user_id) || 'M').charAt(0).toUpperCase()}
                                </AvatarFallback>
                              </Avatar>
