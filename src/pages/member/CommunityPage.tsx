...
export default function CommunityPage() {
  ...
  const { data: posts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from('forum_posts').select('*').eq('category_id', selectedCategory).order('is_pinned', { ascending: false }).order('created_at', { ascending: false });
      if (error) throw error;
  ...
  const createPostMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from('forum_posts').insert({ category_id: selectedCategory!, author_id: user!.id, title: postForm.title, body: postForm.body });
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['forum-posts'] }); queryClient.invalidateQueries({ queryKey: ['forum-post-counts'] }); setNewPostOpen(false); setPostForm({ title: '', body: '' }); toast({ title: 'Post created' }); },
    onError: (e: Error) => toast({ title: 'Error', description: e.message, variant: 'destructive' }),
  });

  const createReplyMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from('forum_replies').insert({ post_id: selectedPost!, author_id: user!.id, body: replyText });
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['forum-replies'] }); setReplyText(''); toast({ title: 'Reply posted' }); },
    onError: (e: Error) => toast({ title: 'Error', description: e.message, variant: 'destructive' }),
  });
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="pb-24 px-6 pt-8 lg:px-12 max-w-2xl mx-auto space-y-6">
        ...
        <Card className="border border-border">
          <CardHeader>
            <div className="flex items-start gap-3">
              <Avatar className="h-9 w-9 mt-0.5"><AvatarFallback className="text-xs bg-primary/10 text-primary">{getInitials(getName(currentPost.author_id))}</AvatarFallback></Avatar>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  {currentPost.is_pinned && <Pin className="h-3.5 w-3.5 text-primary" />}
                  <CardTitle className="text-lg font-[900]">{currentPost.title}</CardTitle>
                </div>
                <p className="text-xs text-muted-foreground mt-1">{getName(currentPost.author_id)} · {format(new Date(currentPost.created_at), 'MMM d, yyyy HH:mm')}</p>
              </div>
            </div>
          </CardHeader>
          <CardContent>
        ...
        <div className="space-y-4">
          ...
          {replies.map(reply => (
            <Card key={reply.id} className="border border-border">
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  <Avatar className="h-7 w-7 mt-0.5"><AvatarFallback className="text-[10px] bg-accent text-accent-foreground">{getInitials(getName(reply.author_id))}</AvatarFallback></Avatar>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
          ...
          <Card className="border border-primary/20">
            <CardContent className="p-4">
              <Textarea value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="Write a reply…" rows={3} />
              <Button size="sm" className="mt-3 rounded-full" onClick={() => createReplyMutation.mutate()} disabled={!replyText.trim() || createReplyMutation.isPending}>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="pb-24 px-6 pt-8 lg:px-12 max-w-2xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => setSelectedCategory(null)}><ArrowLeft className="h-4 w-4 mr-2" />Back</Button>
            <div>
              <h1 className="text-xl font-[900] text-foreground">{currentCategory?.name}</h1>
              {currentCategory?.description && <p className="text-muted-foreground text-sm">{currentCategory.description}</p>}
            </div>
          </div>
          <Dialog open={newPostOpen} onOpenChange={setNewPostOpen}>
            <DialogTrigger asChild><Button size="sm" className="rounded-full"><Plus className="h-4 w-4 mr-1" />New Post</Button></DialogTrigger>
            <DialogContent>
              ...
              <div className="space-y-4 mt-2">
                ...
                <Button onClick={() => createPostMutation.mutate()} disabled={!postForm.title.trim() || createPostMutation.isPending} className="w-full rounded-full">Post Discussion</Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>
        {posts.length === 0 ? (
          ...
          <div className="space-y-2">
            {posts.map(post => (
              <Card key={post.id} className="cursor-pointer hover:bg-muted/50 transition-colors border border-border" onClick={() => setSelectedPost(post.id)}>
                <CardContent className="p-4 flex items-center gap-4">
                  <Avatar className="h-8 w-8"><AvatarFallback className="text-xs bg-primary/10 text-primary">{getInitials(getName(post.author_id))}</AvatarFallback></Avatar>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{getName(post.author_id)} · {format(new Date(post.created_at), 'MMM d')}</p>
                  </div>
                  <div className="flex items-center gap-1 text-muted-foreground text-xs">
                    <Clock className="h-3 w-3" />{format(new Date(post.created_at), 'HH:mm')}
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {categories.map((cat, i) => (
            <motion.div key={cat.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
              <Card className="cursor-pointer hover:bg-muted/50 transition-colors border border-border" onClick={() => setSelectedCategory(cat.id)}>
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    ...
                    <Badge variant="outline" className="text-xs">{(postCounts as Record<string, number>)[cat.id] || 0} posts</Badge>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
